  ...
  connectionPool.getConnection(function(err, connection) {
    if (err) {
      ...
      connection.query(query, function(err, rows, fields) {
        if (err) {
          myConsole.error(
            '[Database] An Error has occured Executing Query: "' + query + '"'
          );
          myConsole.error(err);
          callback({ message: "An Error has occured" });
        } else {
          myConsole.log(
            '[Database] Successfully Executed Query: "' + query + '"'
          );
          callback(rows);
        }
      });
...
connectionPool.invalidValues = (vals, numValsExpected, callback) => {
  if (numValsExpected === -1) {
    myConsole.error('[Database] Invalid value(s) "' + vals + '" encountered.');
  } else {
    if (
      vals === null ||
    ...
    } else if (connectionPool.isArray(vals) && vals.length > numValsExpected) {
...
connectionPool.handleAPI = (
  ...
  if (expectedNumInts === 0 || connectionPool.verifyIntArray(ints)) {
    ...
    if (ints && otherVals) {
      ...
      valArray = connectionPool.isArray(otherVals) ? otherVals : [otherVals];
    }
